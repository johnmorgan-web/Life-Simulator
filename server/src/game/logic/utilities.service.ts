...
export class UtilitiesService {
  ...
  variableMultiplier(
    ...
    const seasonal: Record<string, number[]> = {
      utilities: [
        0.02, 0.03, 0.02, 0.0, -0.01, -0.02, 0.03, 0.03, 0.01, 0.0, 0.01, 0.04,
      ],
      food: [0.0, 0.0, 0.0, 0.0, 0.01, 0.01, 0.0, 0.01, 0.0, 0.0, 0.03, 0.04],
      gas: [0.01, 0.01, 0.0, 0.0, 0.0, 0.03, 0.04, 0.03, 0.01, 0.0, 0.0, 0.0],
      car: [
        0.02, 0.02, 0.01, 0.0, 0.0, -0.01, -0.01, 0.0, 0.01, 0.02, 0.02, 0.02,
      ],
      entertainment: [
        0.0, 0.02, 0.03, 0.02, 0.01, 0.0, -0.01, 0.0, 0.01, 0.02, 0.03, 0.04,
      ],
    };
  ...
  countRelocations(logs: any[]): number {
    return (Array.isArray(logs) ? logs : []).filter((l: any) =>
      String(l?.msg || '').includes('Relocated to '),
    ).length;
  }
  ...
  subscriptionBadgeMilestones() {
    return [
      ...
      { months: 12, id: 'sub-12', name: 'Streaming Sensei', icon: '🎖️' },
      {
        months: 24,
        id: 'sub-24',
        name: 'Legendary Subscription Overlord',
        icon: '👑',
      },
    ];
  }
  ...
  entertainmentHostCount(budget: number): number {
    const thresholds = [
      30, 75, 100, 140, 220, 1200, 5000, 12000, 25000, 50000, 90000, 150000,
    ];
    let count = 0;
  ...
  ticketStubForHostCount(hostCount: number) {
    if (hostCount >= 12)
      return { title: 'Lunar Theme Park Buyout', icon: '🌕' };
    if (hostCount >= 11)
      return { title: 'Orbital Zero-Gravity Party', icon: '🛰️' };
    if (hostCount >= 10)
      return { title: 'Cruise Ship Esports Festival', icon: '🛳️' };
    if (hostCount >= 9)
      return { title: 'Private Island Weekend Carnival', icon: '🏝️' };
    if (hostCount >= 8)
      return { title: 'Stadium Fireworks Spectacular', icon: '🎆' };
    if (hostCount >= 7)
      return { title: 'Desert Supercar Treasure Rally', icon: '🏎️' };
    if (hostCount >= 6)
      return { title: 'Chartered Yacht Game Night', icon: '🛥️' };
    if (hostCount >= 5)
      return { title: 'Private Theme Park After-Hours', icon: '🎢' };
    if (hostCount >= 4)
      return { title: 'Ballpark Gaming Takeover', icon: '🏟️' };
