...
export function resolveDomainKey(label: string): DomainKey {
  ...
  if (
    ...
    return 'legal';
  if (value.includes('education') || value.includes('social'))
    return 'education';
  if (
    ...
    return 'logistics';
  if (value.includes('service') || value.includes('hospitality'))
    return 'service';
  if (value.includes('creative') || value.includes('commercial'))
    return 'creative';
